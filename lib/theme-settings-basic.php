 ...
 * Define Constants
 */
define('pixkit_SHORTNAME', 'pixkit'); // used to prefix the individual setting field id see pixkit_options_page_fields()
define('pixkit_PAGE_BASENAME', 'pixkit-settings'); // the settings page slug

/**
 * Specify Hooks/Filters
 */
add_action( 'admin_menu', 'pixkit_add_menu' );
add_action( 'admin_init', 'pixkit_register_settings' );
 ...
 * @return array
 */
function pixkit_get_settings() {
	
	$output = array();
	
	// put together the output array 
	$output['pixkit_option_name'] 		= 'pixkit_options'; // the option name as used in the get_option() call.
	$output['pixkit_page_title'] 		= __( 'Pixkit Settings Page','pixkit_textdomain'); // the settings page title
	$output['pixkit_page_sections'] 	= pixkit_options_page_sections(); // the setting section
	$output['pixkit_page_fields'] 		= pixkit_options_page_fields(); // the setting fields
	$output['pixkit_contextual_help'] 	= pixkit_options_page_contextual_help(); // the contextual help
 ...
 */
function pixkit_create_settings_field( $args = array() ) {
	...
	$defaults = array(
		...
		'section' => 'main_section', 					// the section this setting belongs to — must match the array key of a section in pixkit_options_page_sections()
	...
	// additional arguments for use in form field output in the function pixkit_form_field_fn!
	$field_args = array(
		'type'      => $type,
		'id'        => $id,
		'desc'      => $desc,
	...
	add_settings_field( $id, $title, 'pixkit_form_field_fn', __FILE__, $section, $field_args );
 ...
 *
 */
function pixkit_register_settings(){
	
	// get the settings sections array
	$settings_output 	= pixkit_get_settings();
	$pixkit_option_name = $settings_output['pixkit_option_name'];
	
	//setting
	register_setting($pixkit_option_name, $pixkit_option_name, 'pixkit_validate_options' );
	
	//sections
	if(!empty($settings_output['pixkit_page_sections'])){
		// call the "add_settings_section" for each!
		foreach ( $settings_output['pixkit_page_sections'] as $id => $title ) {
			add_settings_section( $id, $title, 'pixkit_section_fn', __FILE__);
		}
	}
		
	//fields
	if(!empty($settings_output['pixkit_page_fields'])){
		// call the "add_settings_field" for each!
		foreach ($settings_output['pixkit_page_fields'] as $option) {
			pixkit_create_settings_field($option);
		}
	}
}
 ...
 *
 */
function pixkit_settings_scripts(){
	wp_enqueue_style('pixkit_theme_settings_css', get_template_directory_uri() . '/lib/css/theme_settings.css');
	wp_enqueue_script( 'pixkit_theme_settings_js', get_template_directory_uri() . '/lib/js/theme_settings.js', array('jquery'));
}
 ...
 *
 */
function pixkit_add_menu(){

	$settings_output 		= pixkit_get_settings();
	// collect our contextual help text
	$pixkit_contextual_help = $settings_output['pixkit_contextual_help'];
	
	// Display Settings Page link under the "Appearance" Admin Menu
	// add_theme_page( $page_title, $menu_title, $capability, $menu_slug, $function);
	$pixkit_settings_page = add_theme_page(__('PixKit Options'), __('PixKit Options','pixkit_textdomain'), 'manage_options', pixkit_PAGE_BASENAME, 'pixkit_settings_page_fn');
		// contextual help
		if ($pixkit_settings_page) {
			add_contextual_help( $pixkit_settings_page, $pixkit_contextual_help );
		}
		// css & js
		add_action( 'load-'. $pixkit_settings_page, 'pixkit_settings_scripts' );	
}
 ...
 */
function  pixkit_section_fn($desc) {
	echo "<hr>";
	//echo "<p>" . __('Settings for this section','pixkit_textdomain')  . "</p> ";
}
 ...
 * @return echoes output
 */
function pixkit_form_field_fn($args = array()) {
	...
	$pixkit_option_name = $settings_output['pixkit_option_name'];
	$options 			= get_option($pixkit_option_name);
	...
	switch ( $type ) {
		case 'text':
			...
			echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $pixkit_option_name . "[$id]' value='$options[$id]' />";
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				$item[0] = esc_html__($item[0], 'pixkit_textdomain');
				if (!empty($options[$id])) {
				...
				echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $pixkit_option_name . "[$id|$item[1]]' value='$value' /><br/>";
			}
		...
		case 'textarea':
			...
			echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $pixkit_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
		
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $pixkit_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'pixkit_textdomain');
					$item 	= esc_html($item, 'pixkit_textdomain');
		...
		case 'select2':
			...
			foreach($choices as $item) {
				...
				$item = explode("|",$item);
				$item[0] = esc_html($item[0], 'pixkit_textdomain');
		...
		case 'checkbox':
			echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $pixkit_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
		...
		case "multi-checkbox":
			foreach($choices as $item) {
				...
				echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $pixkit_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
			}
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
	}
 ...
 * @return echoes output
 */
function pixkit_settings_page_fn() {
	// get the settings sections array
	$settings_output = pixkit_get_settings();
?>
	<div class="wrap">
		<div class="icon32" id="icon-options-general"></div>
		<h2><?php echo $settings_output['pixkit_page_title']; ?></h2>
		
		<form action="options.php" method="post">
			<?php 
			// http://codex.wordpress.org/Function_Reference/settings_fields
			settings_fields($settings_output['pixkit_option_name']); 
			// http://codex.wordpress.org/Function_Reference/do_settings_sections
			do_settings_sections(__FILE__); 
			?>
			
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','pixkit_textdomain'); ?>" />
			</p>
 ...
 * @return array
 */
function pixkit_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['pixkit_page_fields'];
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									pixkit_SHORTNAME . '_txt_numeric_error', // error ID
									__('Expecting a Numeric value! Please fix.','pixkit_textdomain'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','pixkit_textdomain');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									$option['id'], // setting title
									pixkit_SHORTNAME . '_txt_multinumeric_error', // error ID
									__('Expecting comma separated numeric values! Please fix.','pixkit_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','pixkit_textdomain');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','pixkit_textdomain');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									$option['id'], // setting title
									pixkit_SHORTNAME . '_txt_email_error', // error ID
									__('Please enter a valid email address.','pixkit_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
 ...
 */
function pixkit_show_msg($message, $msgclass = 'info') {
 ...
 * @since PixKit 0.1.0
 *
 * @return calls pixkit_show_msg()
 */
function pixkit_admin_msgs() {
	
	// check for our settings page - need this in conditional further down
	$pixkit_settings_pg = strpos($_GET['page'], pixkit_PAGE_BASENAME);
	...
	if(current_user_can ('manage_options') && $pixkit_settings_pg !== FALSE && !empty($set_errors)){

		// have our settings succesfully been updated? 
		if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
			pixkit_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
		...
		// have errors been found?
			...
			foreach($set_errors as $set_error){
				...
				pixkit_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
			}
		}
	}
}
...
// admin messages hook!
add_action('admin_notices', 'pixkit_admin_msgs');
?>